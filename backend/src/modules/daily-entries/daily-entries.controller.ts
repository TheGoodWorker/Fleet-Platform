...
export class DailyEntriesController {
  ...
  findByContract(
    ...
    return this.service.findByContract(contractId, filters, page, limit);
  }

  @Get('contract/:contractId/progress')
  @Roles(UserRole.DRIVER)
  @ApiOperation({ summary: 'Résumé de progression d\'un contrat OWNERSHIP_PROGRAM' })
  getProgressSummary(@Param('contractId') contractId: string) {
    return this.service.getProgressSummary(contractId);
  }
